...
- **明确假设**：如果对任务目标、范围或文件归属有不确定，先说明假设再行动；若有多种解读，列出来让用户选，而不是默默选一种。
- **最小改动**：只触碰当前任务直接需要的文件。如果发现不相关的问题（断链、格式错误、孤页），说明但不修复，除非用户明确要求。
- **可验证目标**：动手前将任务转化为可检查的成功标准（见第 7 节）。完成后对照标准自验。
...
每类操作开始前，先将任务转化为可检查的成功标准，完成后对照验证。

### ingest

步骤：先落 `raw/` → 再补 `wiki/sources/` → 必要时更新索引和概念页。

验证标准：
- [ ] `raw/` 中有对应文件，frontmatter 字段齐全（`source_type`、`created_at`、`topics`）
- [ ] `wiki/sources/` 中有来源卡，说明"材料讲了什么、价值是什么、连到哪些概念"
- [ ] 相关索引页有入口，无孤立来源卡
- [ ] `wiki/log.md` 追加了本次操作记录

### query

步骤：先列证据路径（读哪些文件） → 再给判断 → 结果写入 `outputs/`。

验证标准：
- [ ] 结论可追溯到具体来源卡或概念页
- [ ] 结果文件落在 `outputs/answers/` 或 `outputs/syntheses/`
- [ ] 相关索引页有回流入口（如有稳定结论）
- [ ] `wiki/log.md` 追加了本次操作记录

### lint

步骤：检查孤页、断链、缺摘要来源、重复概念、附件缺失、输出未回流。

验证标准：
- [ ] 检查范围已明确（全库 or 指定目录）
- [ ] 问题已列清单，标注类型和位置
- [ ] 本次 lint 未顺手修复无关问题（只报告，修复需另行确认）
- [ ] `wiki/log.md` 追加了本次操作记录

### backfill

步骤：从 `outputs/` 识别高价值结果 → 补索引入口 → 必要时回写概念页。

验证标准：
- [ ] 至少补 1 个索引入口
- [ ] 若结论仍是阶段性，停在 `outputs/` 层，不强制写概念页
- [ ] `wiki/log.md` 追加了本次操作记录
...
- 知识库相关主题使用：`wiki/indexes/llm-wiki/`（含 `LLM-Wiki总索引.md`、`LLM-Wiki来源清单.md`、`LLM-Wiki阅读地图.md`）
...
- 复杂任务至少先读：
  ...
  - 必要的来源卡或概念页
- 能本地完成的任务尽量本地完成。
- **不主动修复与当前任务无关的问题**：发现不相关的断链、孤页、格式问题，说明即可，不要顺手修复。
- **每条改动都应能追溯到用户请求**：如果某行改动无法对应到任务，删掉它。
- 不覆盖、回滚或清理自己未创建的改动。
...
- `wiki/index.md`
- `wiki/schema.md`
- `wiki/indexes/llm-wiki/LLM-Wiki总索引.md`
