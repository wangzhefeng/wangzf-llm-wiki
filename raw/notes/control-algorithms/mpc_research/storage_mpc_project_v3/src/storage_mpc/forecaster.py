...
class NaiveRollingForecaster:
    ...
    def forecast(
        ...
        for k in range(horizon):
            ...
            plan_hat = float(df.loc[idx, "grid_buy_plan_mw"]) + self.rng.normal(0, self.config.forecast_noise.plan_sigma)

            load_pred.append(max(load_hat, 0.0))
            buy_price_pred.append(max(price_hat, 0.01))
            pv_pred.append(max(pv_hat, 0.0))
            plan_pred.append(max(plan_hat, 0.0))

        return (
            np.array(load_pred),
            np.array(buy_price_pred),
            np.array(pv_pred),
            np.array(plan_pred),
        )
