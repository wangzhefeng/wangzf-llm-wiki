...
def generate_synthetic_data(config: ProjectConfig) -> pd.DataFrame:
    ...
    evening_bump = 4.2 * np.exp(-0.5 * ((hour_of_day - 19) / 2.3) ** 2)
    ...
    # 模拟日前购电计划，后续可作为偏差考核基线
    grid_plan = np.clip(load_actual - 0.5 * pv_actual + rng.normal(0, 0.4, size=T), 0.0, None)
