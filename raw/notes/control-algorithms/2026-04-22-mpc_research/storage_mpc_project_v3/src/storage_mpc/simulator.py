...
def run_closed_loop_simulation(df_actual: pd.DataFrame, config: ProjectConfig) -> pd.DataFrame:
    ...
    for t in range(T):
        load_pred, buy_price_pred, pv_pred, plan_pred = forecaster.forecast(df_actual, t, N)
        ...
        plan_actual = float(df_actual.loc[t, "grid_buy_plan_mw"])
        ...
        energy_cost = buy_price_actual * p_buy * dt - sell_price_actual * p_sell * dt
        deviation_cost = config.weights.lambda_dev * (p_buy - plan_actual) ** 2 if config.enable_deviation_penalty else 0.0
        stage_cost = energy_cost + deviation_cost
        ...
        rows.append({
            ...
            "p_buy_mw": p_buy,
            "p_sell_mw": p_sell,
            "peak_buy_value": sol.peak_buy_value,
