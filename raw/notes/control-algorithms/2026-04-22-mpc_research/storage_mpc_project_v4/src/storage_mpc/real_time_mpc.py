    ...
    def solve(
        ...
        for k in range(horizon):
            ...
            cost += (
                ...
                + w.lambda_plan_track * cp.square(p_buy[k] - grid_buy_ref_mw[k])
            )

        cost += w.lambda_terminal * cp.square(soc[horizon] - batt.soc_ref)
        constraints += [soc[horizon] >= batt.soc_min, soc[horizon] <= batt.soc_max]

        prob = cp.Problem(cp.Minimize(cost), constraints)
        prob.solve(solver=cp.OSQP, warm_start=True, verbose=False)

        if prob.status not in ("optimal", "optimal_inaccurate"):
            raise RuntimeError(f"Real-time MPC solve failed: {prob.status}")

        return RealTimeMPCSolution(
