...
description: "Shell 变量 在 Shell 编程中，变量是用于存储数据值的名称。 定义变量时，变量名不加美元符号（$，PHP语言中变量需要），如：\
  \   [mycode4 type='bash'] your_name='runoob' [/mycode4]   注意，变量名和等号之间不能有空格，这可能和你熟悉的所有编程语言都不一样。同时，变量名的命名须遵循如下规则：\
  \   \t \t\t只包含字母、数字和下划线： 变.."
source_type: web
status: inbox
tags:
...
topics:
- 编程教程
source_url: https://www.runoob.com/linux/linux-shell-variable.html
published_at: null
related_concepts: []
---
...
这样的声明告诉 Shell 将 my\_integer 视为整数，如果尝试将非整数值赋给它，Shell会尝试将其转换为整数。
...
your\_name="runoob"  
str="Hello, I know you are \\"$your\_name\\"! \\n"  
echo -e $str  
...
以上例子中， 是一个空命令，用于执行后面的 Here 文档， 表示开启 Here 文档，COMMENT 是 Here 文档的标识符，在这两个标识符之间的内容都会被视为注释，不会被执行。
