...
topics:
- 深度学习
source_url: https://docs.pytorch.org/docs/stable/notes/multiprocessing.html
...
The CUDA runtime has the limitation described in when using the `fork` start method; either the `spawn` or `forkserver` start method are required to use CUDA in subprocesses.
