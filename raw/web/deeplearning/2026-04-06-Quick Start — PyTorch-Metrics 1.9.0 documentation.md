...
topics:
  - 深度学习
status: inbox
source: "https://lightning.ai/docs/torchmetrics/stable/pages/quickstart.html"
published: 
created: 2026-04-06
description: 
tags:
...
Similar to [torch.nn](https://pytorch.org/docs/2.10/nn), most metrics have both a class-based and a functional version. The functional versions implement the basic operations required for computing each metric. They are simple python functions that as input take [torch.tensors](https://pytorch.org/docs/2.10/tensors.html) and return the corresponding metric as a `torch.tensor`. The code-snippet below shows a simple example for calculating the accuracy using the functional interface:
...
Implementing your own metric is as easy as subclassing a [`torch.nn.Module`](https://docs.pytorch.org/docs/stable/generated/torch.nn.Module.html#torch.nn.Module "(in PyTorch v2.10)"). Simply, subclass [`Metric`](https://lightning.ai/docs/torchmetrics/stable/references/metric.html#torchmetrics.Metric "torchmetrics.Metric") and do the following:
