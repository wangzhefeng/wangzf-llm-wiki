...
source_url: https://docs.pytorch.org/docs/stable/elastic/run.html
published_at: 2001-05-04
related_concepts: []
topics:
  - llm-post-training
  - 大语言模型后训练
...
In both cases of single-node distributed training or multi-node distributed training, `torchrun` will launch the given number of processes per node (`--nproc-per-node`). If used for GPU training, this number needs to be less or equal to the number of GPUs on the current system (`nproc_per_node`), and each process will be operating on a single GPU from *GPU 0 to GPU (nproc\_per\_node - 1)*.
...
`--nproc-per-node` may be `"gpu"` (spawn one process per GPU), `"cpu"` (spawn one process per CPU), `"xpu"` (spawn one process per XPU), `"auto"` (equivalent to `"gpu"` if CUDA is available, else equivalent to `"xpu"` if XPU is available, else equivalent to `"cpu"`), or an integer specifying the number of processes. See [torch.distributed.run.determine\_local\_world\_size](https://github.com/pytorch/pytorch/blob/0a94bb432ed75cc2d950d81b2921363218a7e459/torch/distributed/run.py#L673-L716) for more details.
...
2. `--rdzv-backend`: An implementation of [`torch.distributed.elastic.rendezvous.RendezvousHandler`](https://docs.pytorch.org/docs/stable/elastic/rendezvous.html#torch.distributed.elastic.rendezvous.RendezvousHandler "torch.distributed.elastic.rendezvous.RendezvousHandler")
...
See [NUMA Binding](https://docs.pytorch.org/docs/stable/elastic/numa.html#numa-api) for more details.
...
3. `RANK` is NOT stable. Between restarts, the local workers on a node can be assigned a different range of ranks than before. NEVER hard code any assumptions about the stable-ness of ranks or some correlation between `RANK` and `LOCAL_RANK`.
