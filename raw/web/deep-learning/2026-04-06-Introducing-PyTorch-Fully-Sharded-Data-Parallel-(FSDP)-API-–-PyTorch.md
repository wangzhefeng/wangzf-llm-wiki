...
![[raw/assets/attachments/deeplearning/fsdp_workflow 1.png]]
...
fsdp\_auto\_wrap\_policy argument allows specifying a callable function to recursively wrap layers with FSDP. default\_auto\_wrap\_policy function provided by the PyTorch FSDP recursively wraps layers with the number of parameters larger than 100M. You can supply your own wrapping policy as needed. The example of writing a customized wrapping policy is shown in the [FSDP API doc](https://pytorch.org/docs/stable/fsdp.html).

In addition, cpu\_offload could be configured optionally to offload wrapped parameters to CPUs when these parameters are not used in computation. This can further improve memory efficiency at the cost of data transfer overhead between host and device.
...
Manual wrapping can be useful to explore complex sharding strategies by applying `wrap` selectively to some parts of the model. Overall settings can be passed to the enable\_wrap() context manager.
...
![[raw/assets/attachments/deeplearning/175b_throught.png]]

![[raw/assets/attachments/deeplearning/1t_thought.png]]
