---
title: Agentic后训练 - SFT（监督微调）全流程详解
created: 2026-04-15
updated: 2026-04-15
type: source
tags:
  - llm
sources:
  - raw/web/llm/2026-04-06-Agentic后训练-SFT（监督微调）全流程详解.md
status: summarized
---
## 内容摘要
本文将从Agentic SFT入手，介绍SFT如何赋能智能体（Agent），从而真正使得大模型能够创造生产力。

## 关键要点
- 笨鸟先飞 *2026年1月8日 19:50*
- *原文：https://zhuanlan.zhihu.com/p/1981322057150141868*
- 在大模型技术飞速发展的现在，学术界和工业界已有很多针对大模型监督微调（Supervised Fine-Tuning）的详细调研和总结。

## 来源信息
- 原始文件：raw/web/llm/2026-04-06-Agentic后训练-SFT（监督微调）全流程详解.md
- 来源类型：web
