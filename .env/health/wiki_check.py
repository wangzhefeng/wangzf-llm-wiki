...
EXPECTED_INDEXES = set(EXPECTED_SOURCES)
...
def check_wikilinks(root: Path, wiki_root: Path) -> CheckResult:
    ...
    result = CheckResult("wikilinks", errors[:50], warnings[:50])
    # 添加统计信息
    result.stats = {
...
def check_raw_frontmatter(root: Path, raw_root: Path) -> CheckResult:
    ...
    return CheckResult("raw frontmatter", errors[:50])
...
def check_raw_naming(root: Path, raw_root: Path) -> CheckResult:
    ...
    return CheckResult("raw naming", errors[:50])
...
def check_missing_attachments(root: Path, wiki_root: Path, raw_root: Path) -> CheckResult:
    ...
    return CheckResult("missing attachments", errors[:50])
...
def print_summary(results: dict, root: Path):
    ...
    for name, result in sorted(results.items()):
        ...
        if error_count > 0 or warning_count > 0:
            print(f"{name:20} 错误: {error_count:3d}  警告: {warning_count:3d}")
        else:
            print(f"{name:20} ✅ 通过")
    ...
    for name, result in sorted(results.items()):
        ...
        if result.errors:
            print("  错误:")
            for err in result.errors:  # 显示所有错误
                print(f"    - {err}")
        
        if result.warnings:
            print("  警告:")
            for warn in result.warnings:  # 显示所有警告
                print(f"    - {warn}")
